...
def bleu(trans, refs, refs_lengths, bp="closest", smooth=False, n=4, weights=None):
    p_norm = [0 for _ in range(n)]
    p_denorm = [0 for _ in range(n)]

    # Cut refs by their lengths information.
    refs_cut = []
    for i in range(len(refs)):
        ref_cut = []
        print(refs[i], refs_lengths[i])
        for r, l in zip(refs[i], refs_lengths[i]):
            r = r[:l]
            ref_cut.append(r[:l])
        refs_cut.append(ref_cut)
    refs = refs_cut
...
if __name__ == '__main__':
    trans = [[0, 19618, 5618, 13342, 2324, 8984, 7744, 9, 8, 1], [[0, 19618, 5618, 13342, 2324, 8984, 7744, 9, 8, 1]]]
    refs = [[[0, 19618, 5618, 13342, 2324, 1,3,3,3], [0, 19618, 4, 8, 5, 10,8,4,2,7,6,1,3,3,3]]] # you don't need to care padding!! because bleu = precision?
    ref_lengths = [[6,12]]
    # refs = [[[1,2,3]]]
    print(bleu(trans, refs,ref_lengths)) # , [10], [8,12])
    # 0.598805964232
    # 0.349833012527
