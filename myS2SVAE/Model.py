...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input_idx, hidden, sent_length = None, sort = False):
        ...
        if np.array(sent_length).all() != None:
            ...
            output_unpack, _ = torch.nn.utils.rnn.pad_packed_sequence(sequence = output,
    ...
    def forward(self, input_idx, contexts, context_lengths, encoder_ht, padding = True, use_z_embedding = False, z_embedding = None):
        ...
        if self.use_bidiretional_encoder:
            if encoder_ht.size(1) != 1:
                ...
                hidden = F.tanh(self.encoder2decoder(encoder_ht))
            else:
                hidden = encoder_ht
            if self.attention.attn_type == 'dot':
                contexts = F.tanh(self.encoder2decoder(contexts))  # If use dot attention, need to transform encoder hidden to decoder hidden space
        else:
            hidden = encoder_ht
...
class AttnDecoderRNN_vae(nn.Module):
    def __init__(self, embedding_size, hidden_size, vocab_size, z_size, encoder_layer, attn_obj = None, embedding_model = None, n_layers=1, dropout_p=0.0, max_length=MAX_LENGTH,
                 use_cuda = True, batch_first = False, padding_idx = 1, context_gate = None, use_bidiretional_encoder = False, rnn_style = 'gru',
                 decoder_input_dropout = 0.0, use_vae_attention = False, vae_attention = None, vae_first_h = False,
                 ):
    ...
    def forward(self, input_idx, contexts, context_lengths, encoder_ht, z, padding = True,
        ...
        if self.use_vae_attention or self.vae_first_h:
        ...
        if self.use_bidiretional_encoder:
            if encoder_ht.size(1) != 1:
                ...
                hidden = encoder_ht
            contexts = F.tanh(self.encoder2decoder(contexts))
        else:
            hidden = encoder_ht

        if (first_step) and self.vae_first_h:
            # print('hidden:', hidden.size())
            hidden = hidden + F.tanh(z_trans)
            # print('hidden:', hidden.size())
...
class gmmvae(nn.Module):
    ...
    def init_gaussion(self, init_method = 'randn'):
        ...
        if init_method == 'randn':
            ...
            gaussion_vars_torch = torch.ones(self.k, self.z_size) * 0.2 # for low dimension: suggest * 0.1; for higher dimension of z, suggest lager variance.
        ...
        self.gaussion_vars = torch.nn.Parameter(gaussion_vars_torch, requires_grad=True)  # change: False
    ...
    def encoding(self, encoder_input, decoder_input, context_lengths_src, context_lengths_tgt, h0=None,
                 detach = True, cal_tgt = False):
        # VAE encoding part:
        if h0 is None:
            h0 = self.encoder_hidden

        if self.share_encoder:
            encoder_hs_src, encoder_ht_src_ori = self.encoder(encoder_input, h0, context_lengths_src)
        else:
        ...
        if self.training or cal_tgt: #  encoder_ht_src_ori
    ...
    def get_gaussian_parameters(self, encoding_outputs, fix_logvar = True):
        if self.training:
        ...
        if fix_logvar:
            ...
            logvar = Variable(torch.ones(mu.size()) * 0.1).cuda().log()
    ...
    #     input_idx = args[0]
    #     tgt_idx = args[1]
    #     context_lengths_src = args[2]
    #     context_lengths_tgt = args[3]
    #     src_cls = args[4]
    #     tgt_cls = args[5]

        # print('mus', self.gaussion_mus)
        # print('vars', self.gaussion_vars)
        encoder_input = input_idx
        decoder_input = tgt_idx
        ...
        # encoder_ht_src = encoder_ht_src.contiguous().view(encoder_ht_src.size(0), 1, -1)
        # representation_src = self.h2z(encoder_ht_src)
        # representation_src = self.active_func(representation_src) if (self.active_function != 'None') else representation_src
        #
        # if self.training:
        #     _, encoder_ht_tgt_ori = self.encoder(decoder_input, self.encoder_hidden, context_lengths_tgt, sort = False)
        #     encoder_ht_tgt = torch.chunk(encoder_ht_tgt_ori, self.encoder_n_layer, dim=1)[-1]  # Only consider the last layer!
        #     encoder_ht_tgt = encoder_ht_tgt.contiguous().view(encoder_ht_tgt.size(0), 1, -1)
        #     representation_tgt = self.h2z(encoder_ht_tgt)
        #     representation_tgt = self.active_func(representation_tgt) if (self.active_function != 'None') else representation_tgt
        ...
        if self.training:
            if tgt_cls is not None:
                ...
                cludis_tgt = self.cluster_prob(representation_tgt)
            # print(representation_tgt[0].data)
            # print(representation_tgt[0])
            # print(cludis_tgt.data)
            # print(representation_tgt)
            # topv, topi = cludis_tgt.data.topk(1)
            # print(topi.cpu().tolist())
            # print(representation_tgt)
            # print('gaussian', self.gaussion_vars)
            # print("===")
            # print(cludis_tgt.data.cpu()[0].tolist())
...
class gmmvae_loss(nn.Module):
    ...
    def cluster_kl(self, tgt_probs, src_probs, max_clu_and_detach = False):
        ...
        if not max_clu_and_detach:
            # batch x k
            res = torch.sum(tgt_probs * (tgt_probs / src_probs).log(), dim=1)
        else:
            # pass
    ...
    def forward(self, log_softmax_output, target_output, tgt_probs, src_probs,
        ...
        ckl = torch.sum(self.cluster_kl(tgt_probs, src_probs)) / self.batch_size
