...
sys.path.insert(0, '/home/shiwenxian/vae/myS2SVAE')
print(sys.path)
import torch
...
def read_parameters(config_file = None):
    ...
    if config_file != None:
        ...
        model_name = 'seq2seq'
        fn_quora = {
            'trn_src': '../data/paraphrase_generation/clause.align.v5.train.src',
            'trn_tgt': '../data/paraphrase_generation/clause.align.v5.train.tgt',
            'dev_src': '../data/quora_duplicate_questions_dev.src',
            'dev_tgt': '../data/quora_duplicate_questions_dev.tgt',
            'test_src': '../data/paraphrase_generation/testset.txt.combine.src',  # '../data/quora_duplicate_questions_test.src'
            'test_tgt': '../data/paraphrase_generation/testset.txt.combine.tgt'  # '../data/quora_duplicate_questions_test.tgt'
        }
        fn_quora_unparallel = {
    ...
    with torch.cuda.device(gpu_idx):
        ...
        if Train:
            ...
            if parameters['pretrain_s2s']:
                ...
                s2s_model.load_state_dict(torch.load(parameters['s2s_model_fn'] + ".md"))
