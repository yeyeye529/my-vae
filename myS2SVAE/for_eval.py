...
def read_parameters(config_file = None):
    ...
    if config_file != None:
        ...
        parameters = {
            ...
            'batch_size': 32,  # Quaro: 64 MSCOCO: 128(try)
            'epoch_num': 80,  # 训练轮数
            ...
            'embedding_size': 300,
            'hidden_size': 600,
            'learning_rate': 1e-4,  # * ADAM: [1e-3], 5e-4; SGD: 1e-2
            'drop_out': 0.3,
            'verbose_freq': 500,  # 隔多少个batch输出一次信息
            'stop_threshold': 5e-4,  # 这个现在没有用。
            ...
            'decoder_rnn_style': 'lstm',  # encoder的rnn类型，可以用gru或lstm
            'encoder_rnn_style': 'lstm',  # decoder的rnn类型，同上。
            ...
            'vae_attention': False,  # 是否用vae attention
        ...
        model_name = 'VAE'
        fn_quora = {
            'trn_src': '../data/quora_duplicate_questions_trn_150K.src',
            'trn_tgt': '../data/quora_duplicate_questions_trn_150K.tgt',
            'dev_src': '../data/quora_duplicate_questions_dev.src',
            'dev_tgt': '../data/quora_duplicate_questions_dev.tgt',
            'test_src': '../data/quora_duplicate_questions_test_4K.src',  # '../data/quora_duplicate_questions_test.src'
            'test_tgt': '../data/quora_duplicate_questions_test_4K.tgt'  # '../data/quora_duplicate_questions_test.tgt'
        }
        fn_quora_unparallel = {
